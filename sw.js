const SHELL_CACHE = 'athar-shell-v8';
...
self.addEventListener('message', (event) => {
  ...
  event.waitUntil(
    Promise.all([caches.open(SHELL_CACHE), fetch(withBasePath('/fixtures/toulouse.pmtiles'), { cache: 'no-store' })])
      .then(([cache, response]) => {
        if (!response.ok) throw new Error(`PMTiles download failed: ${response.status}`);
        return cache.put(withBasePath('/fixtures/toulouse.pmtiles'), response);
      })
